...
CAP runs the container on a read-only rootfs inside a confidential guest. The image needs to meet these requirements:
...
Leaving out `[storage]` does not turn storage off: CAP still defaults to `paths = ["/data"]`. If the app needs no persistent storage, set `paths = []` explicitly and skip the `VOLUME`.
...
- **Browser rejects the app cert (`ERR_CERT_AUTHORITY_INVALID`)**: some environments serve a Let's Encrypt staging certificate, which is a platform troubleshooting toggle. Use `curl -sk`, or use an environment with a production Let's Encrypt certificate.
